<x-layout>
    ...
    <x-sub-header>
        <div class="px-5">
            <span class="font-daysone text-blue-900">INSPECTORS</span>
        </div>
    </x-sub-header>
    ...
    <x-containers.main>
        <div class="w-11/12 mx-auto my-3 flex border-b border-gray-300 pb-2">
            <div class="font-montserrat flex">
                <a href="{{ route('administrators.inspectors.create') }}" class="text-blue-600 border-2 border-blue-600 bg-white focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-xs px-4 py-1.5 text-center mr-2 mb-2 flex items-center space-x-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zM4 19.235v-.11a6.375 6.375 0 0112.75 0v.109A12.318 12.318 0 0110.374 21c-2.331 0-4.512-.645-6.374-1.766z" />
                    </svg>

                    <span>Create Inspectors</span>
                </a>
            </div>
        </div>
        ...
        <x-containers.content>
            <div>
                <h2 class="font-montserrat px-5 py-5 border-b text-center">
                    <span class="font-semibold text-blue-900">EDIT INSPECTOR PROFILE</span>
                </h2>
            </div>
    ...
    <form action="{{ route('administrators.inspectors.update', $inspector->id) }}" method="POST" id="create-establishment">
        @csrf
        @method('PATCH')
    </form>
